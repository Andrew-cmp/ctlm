...
using namespace std;
//https://github.com/BBuf/how-to-optim-algorithm-in-cuda/tree/master/softmax
...
//这段代码实现了一个高效的数据加载器，主要用于 GPU 内核中的向量化内存访问（如 CUDA），
//目的是将 SRC 类型的数据从全局内存高效加载到 DST 类型的数组中，通过合并内存访问（Coalesced Memory Access）提升性能。以下是详细分析：
...
// rows_per_access 代表每个线程组一次处理的行数，当cols较小，thread_group_width不是warp_size 32时，
...
//这段代码定义了一个模板结构体 DispatchSoftmaxWarpImplPackSize
//其核心功能是 根据输入数据的列数（cols）动态选择最优的向量化打包策略（pack size），以在 GPU 上高效执行 Softmax 计算。
...
//std::is_same<ComputeType, double>::value ：检查 ComputeType 是否不是 double。如果不是，则启用这个函数模板。
...
//std::is_same<ComputeType, double>::value ：检查 ComputeType 是否为 double。如果是，则启用这个函数模板。
template<typename LOAD, typename STORE, typename ComputeType>
inline typename std::enable_if<std::is_same<ComputeType, double>::value, cudaError_t>::type
DispatchSoftmax(cudaStream_t stream, LOAD load, STORE store, const int64_t rows,
                const int64_t cols) {
...
int main(){
  ...
  //始化了一个 DirectLoad 对象 load，用于从全局内存（如 GPU 显存）中高效加载 float 类型的数据，并将其转换为 ComputeType 类型的目标数
