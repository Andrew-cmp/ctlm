...
__global__ void mmaPermuted_Coalesced_32Stride(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C,
    ...
    // printf("in mmaPermuted_Coalesced_32Stride\n");
    ...
    for(int stage_load_id = 0; stage_load_id < (K_STAGE - 1); ++stage_load_id){ // read 3 buffer
        ...
        for (size_t i = 0; i < A_smem_iters; ++i) {
            /*
            half[8 x 32], 16 bytes per thread with cp.async 
            0   1   2   3
            4   5   6   7
            11  8   9   10
            15  12  13  14
            ...
            29  30  31  28
            */
           // CHUNK_COPY_LINE_LANES * SMEM_BANK_ROWS = 8个线程铺满一个bank, 8行交错一个迭代
           // 交错范围是CHUNK_COPY_LINE_LANES
           // 交错间距是SMEM_BANK_ROWS
            ...
            //     if(blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && warp_id == 0 && lane_id == tt){
        ...
        for (size_t i = 0; i < B_smem_iters; ++i) {
            ...
            // if(i == 0 && stage_load_id == 0)
            // for(int tt = 0; tt < 32; ++tt){
            //     if(blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && warp_id == 0 && lane_id == tt){
                    
            //         printf("%d ", lane_id);
            //         printf("%d\t", B_smem_ptr - B_smem_idx_off * AB_SMEM_STRIDE * sizeof(half));
            //     }
            // }
            // __syncthreads();
    ...
    for (size_t i = 0; i < WARP_COL_TILES; ++i) {
        ...
        //             printf("%d ", A_smem_idx + lane_id % 16);
        //             printf("%d ", ((lane_id / 16) * 8 + 
        //                 (lane_id % 16 % (ARRAY_OFFSET_INSMEM)) / SMEM_BANK_ROWS * PERMUTED_OFFSET) %
        //                 AB_SMEM_STRIDE);
        //             printf("%d\t", A_smem_lane_addr);
        //         }
        //     }
        // }
    ...
    for (size_t j = 0; j < WARP_ROW_TILES; j ++) {
        ...
        //             printf("%d ", B_smem_idx + lane_id % 8 - B_smem_idx_off);
        //             printf("%d ", ( ((lane_id / 8) % 2) * 8 +
        //             (lane_id % 16 % (ARRAY_OFFSET_INSMEM)) / SMEM_BANK_ROWS * PERMUTED_OFFSET) %
        //             AB_SMEM_STRIDE);
        //             printf("%d\t", B_smem_lane_addr);
        //         }
        //     }
        // }
...
//write to smem to keep coaselced
    ...
    #pragma unroll
        for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
            ...
            //     if(blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && warp_id == 0 && lane_id == tt){
            //         printf("%d ", lane_id);
            //         printf("%d ", ((i * MMA_M + lane_id / 4) * C_SMEM_STRIDE +
            //                     ((warp_id % BLOCK_ROW_WARPS) * C_SMEM_OFFSET + j * MMA_N +
            //                     (lane_id % 4) * sizeof(int) / sizeof(half) + ((lane_id / 4) % 8) * PERMUTED_OFFSET) %
            //                     C_SMEM_STRIDE) / 128);
            //         printf("%d \t", ((i * MMA_M + lane_id / 4) * C_SMEM_STRIDE +
            ...
            //                     (lane_id % 4) * sizeof(int) / sizeof(half) + ((lane_id / 4) % 8) * PERMUTED_OFFSET) %
            //                     C_SMEM_STRIDE) % 128);
            //         // printf("%d\t",(i * MMA_M + lane_id / 4 + 8) * C_SMEM_STRIDE +
            //         //             ((warp_id % BLOCK_ROW_WARPS) * C_SMEM_OFFSET + j * MMA_N +
            //         //             (lane_id % 4) * sizeof(int) / sizeof(half) + ((lane_id / 4) % 8) * PERMUTED_OFFSET) %
            //         //             C_SMEM_STRIDE);
            //     }
    ...
    for (size_t i = 0; i < (write_warp_lines / write_per_warp_line); ++i) {
        *((int4 *)(write_back_c_ptr + (i * write_per_warp_line + lane_id / write_back_line_thread) * N) + lane_id % write_back_line_thread)
            ...
            *((int4 *)(smem_load_to_c + (i * write_per_warp_line + lane_id / write_back_line_thread) * C_SMEM_STRIDE) +
            ...
            //     if(blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0 && warp_id == 0 && lane_id == tt){
            //         printf("%d ", lane_id);
            //         printf("%d ", ((i * write_per_warp_line + lane_id / write_back_line_thread) * N) + lane_id % write_back_line_thread * 8) ;
            //         printf("%d \t", ((i * write_per_warp_line + lane_id / write_back_line_thread) * C_SMEM_STRIDE) +
            //                         (lane_id % write_back_line_thread + (i * write_per_warp_line + lane_id / write_back_line_thread) % 8) % (C_SMEM_STRIDE * sizeof(half) / THREAD_COPY_BYTES) * 8);
            //         // printf("%d\t",(i * MMA_M + lane_id / 4 + 8) * C_SMEM_STRIDE +
            //         //             ((warp_id % BLOCK_ROW_WARPS) * C_SMEM_OFFSET + j * MMA_N +
            //         //             (lane_id % 4) * sizeof(int) / sizeof(half) + ((lane_id / 4) % 8) * PERMUTED_OFFSET) %
            //         //             C_SMEM_STRIDE);
...
int main(int arg, char* argv[]){
    ...
    for (int j = 4; j < test_num; j++){
        ...
        //testF16F16GemmMaxError(myF16F16GemmTCWarp<HGEMMAlignedV2>, M, N, K);
    }
    return 0;
}