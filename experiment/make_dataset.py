...
def for_gen_tokenizer(work_dir):
    ...
    target_info = target_info(virtual_target)
    ...
    for line, insts_part, decisions_part, decisions_label, parallel_label, latency in for_init_lines(lines, path_tuning_record):
        ...
        data_line = {'text': [task_name_part, shape_part, target_part, insts_part, decisions_part,
                    ...
                    'latency': latency}
        sorted_data_line = sorted(data_line,key = lambda x: x["latency"])
        for rank, item in enumerate(sorted_data_line, start=1):  # 从 1 开始排名
            item["rank"] = rank
        # 删除 latency 键值对
        for item in sorted_data_line:
            item.pop("latency", None)  # 使用 pop 安全删除 "latency" 键
...
def main():
    ...
    if script_args.for_type == FOR_GEN_TOKENIZER:
        dataset_dirs = os.listdir(script_args.dataset_path)
        all_dirs = []
        for dataset_dir in dataset_dirs:
            all_dirs.extend(get_all_dirs(dataset_dir))
