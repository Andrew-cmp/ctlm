...
def worker(gpu_id, lock):
    ...
    while True:
        ...
        if to_measure_file:
            ...
            while True:
                command = f"CUDA_VISIBLE_DEVICES={gpu_id} python measure_programs.py --result_error_threshold=3 --moved_dir={moved_dir} --target=\"nvidia/nvidia-a6000\" --candidate_cache_dir={to_measure_dir_file} --result_cache_dir={measured_tmp_file} >> run_{part_id}.log 2>&1"
