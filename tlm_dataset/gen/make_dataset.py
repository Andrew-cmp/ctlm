...
# 这个函数并没有生成模板，而是对to_measure_programs中的数据进行了处理，比如截断到ppt，比如sp参数都换成1.
# 调度已经在to_measure_programs中了。
def for_gen_eval_sketch(lines, keep_cnt, for_type):
    ...
    # 在latency=0时，json_line_dict中只会有一个元素
    ...
    #这段代码的整体功能是将 latency_list 中的延迟值转换为概率分布，生成的 probs 数组中的值表示每个延迟对应的概率。
    # 这种转换使得后续使用时（如在 np.random.choice 中）可以根据这些概率进行抽样。
    probs = softmax(np.array(latency_list), temperature=0.3)
    #这段代码的整体功能是从 latency_list 的索引中随机选择 keep_cnt 个索引，并根据 probs 中的概率分布进行选择，允许重复选择相同的索引。
    # 换句话说，这段代码可以用于根据某种概率分布在 latency_list 中进行有偏抽样。
...
def main():
    ...
    tasks = load_and_register_tasks()
        ##NLP语言模型的分词器tokenizer是需要训练和学习的，训练了之后可以保存、加载。
        # 这里使用to_measure_programs中的数据集训练这个tokenizer。
    if script_args.for_type == FOR_GEN_TOKENIZER:
        ...
        files.sort()
        print("Dataset file cnt:", len(files))
        #generate prompts for bert-base
        if script_args.for_type == FOR_GEN_EVAL_SKETCH_ONLY_BERT:
            ## to_measure_programs文件夹里面的文件包含所有的sketch，这里只测试bert的，所以只要bert算子的sketch
            ## 关联hold_out_task_files(target, only_bert=False/true):
